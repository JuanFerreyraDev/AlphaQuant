"""Experiment 01: trend_htf feature — Control vs Treatment walk-forward comparison.

Runs 8 walk-forward validations:
  4 CONTROL (current feature set, NO trend_htf):
    C1. BTC_USDT / 4h  × binary_homerun
    C2. BTC_USDT / 4h  × multiclass_3
    C3. BTC_USDT / 1h  × binary_homerun
    C4. BTC_USDT / 1h  × multiclass_3
  4 TREATMENT (current feature set + ["trend_htf"]):
...
Run:  python3 tools/legacy_archive/exp01_trend_htf_walkforward.py
"""
...
def build_full_df(symbol: str, timeframe: str) -> tuple[pd.DataFrame, list[str]]:
    ...
    df, has_trend_htf = add_trend_htf(df, df_1d)
    assert has_trend_htf, f"trend_htf not added for {symbol}/{timeframe}"
    ...
    # run_walk_forward needs these columns untouched for the simulator:
    #   close, atr_14, target
    # and we also need all feature columns (technicals, sentiment, trend_htf).
    # So instead of cleanup_columns (which drops close/atr_14/OHLCV), manually:
    #   (a) drop only the pure-price columns we don't need,
    #   (b) keep close/atr_14/target,
    #   (c) then drop rows with NaN (exactly what cleanup_columns(drop_nan=True) used to do.
    cols_to_drop = ("open", "high", "low", "volume", "ema_50", "vol_sma_20",
                    "max_high_future", "min_low_future")
    df.drop(columns=[c for c in cols_to_drop if c in df.columns], inplace=True)
    df.dropna(inplace=True)

    # Current ("FRANKENSTEIN") feature set: every numeric column that's NOT
    # close (simulator-only price column) / target (y) / trend_htf (treatment).
    # atr_14 stays in features — the model uses it as a signal AND the
    # simulator reads it directly (that's fine, two independent consumers).
    current_features = [
        c for c in df.columns
        if c not in ("close", "target", "trend_htf")
    ...
    assert "trend_htf" in df.columns, "trend_htf column not present after add_trend_htf"
...
def main() -> None:
    ...
    print("=" * 80)
    print("EXPERIMENTO 01 — trend_htf (distancia % a EMA200 diaria)")
    ...
    # Pre-compute the master DataFrames per timeframe (one 4h, one 1h) so
    # control & treatment share EXACTLY the same rows, targets, and
    # technical/sentiment values — the ONLY difference is trend_htf.
    ...
    for tf in TIMEFRAMES:
        ...
        print(f"  ✓ {len(df)} rows × {len(features_current)} current features  "
              f"(trend_htf extra: {'trend_htf' in df.columns})")
        print(f"    Range: {df.index.min().date()} → {df.index.max().date()}")

    all_results: list[dict[str, Any]] = []

    # ------------------------------------------------------------------
    # Execute every (tf × formulation × [control, treatment]) combination
    # ------------------------------------------------------------------
    run_counter = 0
    for tf in TIMEFRAMES:
        df, features_current = master_dfs[tf]
        features_with_htf = features_current + ["trend_htf"]

        for form_name, train_fn, grid in FORMULATIONS:
            for variant, features in [
                ("CONTROL (no trend_htf)", features_current),
                ("TREATMENT (+ trend_htf)", features_with_htf),
            ]:
                run_counter += 1
                ...
                print(f"  Features: {len(features)} cols "
                      ...
                      f"{len(features) - len(features_current)} trend_htf)")
    ...
    # Pairwise deltas (treatment − control) for ΔPF p5 and trade count
    ...
    # Persist results
    # ------------------------------------------------------------------
    out_path = Path(__file__).resolve().parent / "exp01_trend_htf_results.json"
