"""Experiment 03: taker_buy_ratio feature — Control vs Treatment walk-forward.

Runs 8 walk-forward validations:
  4 CONTROL (current 14-col baseline, no trend_htf / no funding_rate_current):
    C1. BTC_USDT / 4h  × binary_homerun
    C2. BTC_USDT / 4h  × multiclass_3
    C3. BTC_USDT / 1h  × binary_homerun
    C4. BTC_USDT / 1h  × multiclass_3
  4 TREATMENT (baseline + ["taker_buy_ratio"]):
    T1. BTC_USDT / 4h  × binary_homerun
    T2. BTC_USDT / 4h  × multiclass_3
    T3. BTC_USDT / 1h  × binary_homerun
    T4. BTC_USDT / 1h  × multiclass_3

All runs use identical configuration (apples-to-apples with exp01 / exp02):
  - swing_period = 10 bars
  - tp_multi     = 1.5 × ATR_14
  - sl_multi     = 1.0 × ATR_14
  - window_months = 6, step_months = 6
  - fee_rate = 0.0, slippage = 0.0
  - threshold_grid = per-formulation standard
  - n_bootstrap = 1000, n_blocks = 8, random_state = 42

NOTE: trend_htf and funding_rate_current are intentionally EXCLUDED from
the baseline — both failed the gate in their experiments.  This
experiment isolates the marginal contribution of taker_buy_ratio alone.

Run:  python3 tools/diagnostics/exp03_taker_buy_ratio_walkforward.py
"""
...
def build_full_df(symbol: str, timeframe: str) -> tuple[pd.DataFrame, list[str]]:
    """Load REST-CSV → technicals → sentiment → taker_buy_ratio → targets.

    Returns:
        (df_with_targets_and_all_features, current_feature_set_list)
        ``current_feature_set_list`` contains every numeric feature EXCEPT
        ``taker_buy_ratio`` (the caller adds it for the TREATMENT arm).

    NOTE: trend_htf and funding_rate_current are intentionally NOT
    included here (both discarded after exp01 / exp02).
    """
    ...
    assert has_taker, (
        f"taker_buy_ratio not computed for {symbol}/{timeframe}. "
        f"Re-download CSV with --binance-rest to include taker fields."
    )
    ...
    cols_to_drop = (
        "open", "high", "low", "volume", "ema_50", "vol_sma_20",
    ...
    current_features = [
        ...
        if c not in ("close", "target", "taker_buy_ratio")
    ...
    assert "taker_buy_ratio" in df.columns, "taker_buy_ratio missing after add_taker_buy_ratio"
...
def main() -> None:
    ...
    print("=" * 80)
    print("EXPERIMENTO 03 — taker_buy_ratio (desequilibrio taker buy/sell por barra)")
    ...
    for tf in TIMEFRAMES:
        ...
        print(f"  ✓ {len(df)} rows × {len(features_current)} current features  "
              f"(taker_buy_ratio extra: {'taker_buy_ratio' in df.columns})")
    ...
    for tf in TIMEFRAMES:
        df, features_current = master_dfs[tf]
        features_with_taker = features_current + ["taker_buy_ratio"]

        for form_name, train_fn, grid in FORMULATIONS:
            for variant, features in [
                ("CONTROL (no taker ratio)", features_current),
                ("TREATMENT (+ taker_buy_ratio)", features_with_taker),
            ]:
                run_counter += 1
                ...
                print(f"  Features: {len(features)} cols "
                      ...
                      f"{len(features) - len(features_current)} taker)")
    ...
    out_path = Path(__file__).resolve().parent / "exp03_taker_buy_ratio_results.json"
